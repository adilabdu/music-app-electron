<template>
  ...
  <div v-if="!loading" class="tracklist-table flex flex-col w-full min-h-[36px] px-[40px] my-[40px]">
    ...
    <div v-for="track in data" class="group hover:!bg-[#353535] track w-full rounded-md flex items-center pl-[7px] pr-[18px] text-[13px] font-light h-[46px]">
      ...
      <div class="relative flex items-center">
        <PlayingIcon :class="[(5 === 6) ? 'opacity-100' : 'opacity-0']" class="absolute ml-1" />
        <p :class="[(5 === 5) ? '!opacity-100' : '!opacity-0']" class="max-w-fit pl-2 pr-6 text-center text-[#FFFFFFA3] group-hover:opacity-0 opacity-100">
          {{ track[0] }}</p>
      </div>
