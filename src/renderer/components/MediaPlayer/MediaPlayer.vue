<template>
  ...
  <div id="mediaPlayer" class="h-full flex items-center max-w-[1600px] grow lg:px-[40px]">
    ...
    <div id="mediaControls" class="hidden lg:flex gap-3 items-center justify-center w-full h-full">
      ...
      <PauseIcon v-else :class="'fill-[#EFEFEE]'" :width="18" @click="play()" />
      <SkipIcon :class="'fill-[#EFEFEE]'" :width="28" />

    </div>

    <div id="track" class="h-full w-full flex items-center justify-center mx-[5px] lg:mx-0">
      <div id="trackArtwork" class="rounded-l-sm h-[44px] w-[44px] bg-[#323232]">
        <img :src="cover" alt="Now playing" class="w-full object-cover rounded-l-sm">
      </div>
      <div id="trackInfo" class="group custom-group relative flex flex-col items-center rounded-r-sm h-[44px] max-w-full lg:max-w-[628px] grow bg-[#4D4D4D]" :style="{ width: (width * 0.35) + 'px' }">
        <audio ref="audio" class="" />

        <div class="w-full flex flex-col items-center justify-center gap-1 mt-[4px]">
          ...
          <p class="text-[13px] text-[#FFFFFFA3] leading-none font-light">
            <a href="#" class="hover:underline" id="trackArtist">Harry Styles</a>
            &#65293;
            <a href="#" class="hover:underline" id="trackAlbum">Harry's House</a>
          </p>
        </div>
        ...
        <input id="trackElapsed" type="range" :min="0" :max="duration" v-model="currentTime" class="z-10 absolute w-full rounded-br-sm h-1 left-0 bottom-0 appearance-none"
    ...
    <div id="mediaVolume" class="hidden lg:flex lg:w-full h-full">
      ...
      <div id="volumeBar" class="grow gap-1 flex items-center justify-center rounded-full">
        ...
        <VolumeIcon :class="'fill-[#818181]'" :width="16" :volume-level="volumeInPercent" />
        <input id="volume" type="range" :min="0" :max="100" v-model="volumeInPercent" class="rounded-sm w-[70px] h-[0.1875rem] left-0 bottom-0 appearance-none"
               :style=" { background: volumeInPercent === 0 ? '#5F5F5F' : 'linear-gradient(to right, #B8B8B8 ' + volumeInPercent + '%, #5F5F5F ' + volumeInPercent + '%)' } ">
...
<script setup>
  ...
  import currentTrack from "../../static/tracks/asitwas.mp3";
  import cover from "../../static/artworks/cover.webp";
  ...
  function play() {
    console.log('hello', playing.value)
    playing.value =! playing.value
    console.log('mars', playing.value)
  }
  ...
  const volumeInPercent = ref(50)
  watch(volumeInPercent, () => {
    volume.value = volumeInPercent.value / 100
  })

  const trackStyle = toRefs({
    background: currentTimeInPercent.value === 0 ?
        '#5F5F5F' : 'linear-gradient(to right, #B8B8B8 ' + currentTimeInPercent.value + '%, #5F5F5F ' + remainingTimeInPercent.value + '%)'
  })

  watch(remainingTimeInPercent, () => {
    console.log('remaining time:', remainingTimeInPercent.value)
    console.log('current time:', currentTimeInPercent.value)
  })
